// versions/common/build.gradle.kts
plugins {
    `java-platform`
    `maven-publish`
    id("com.sphereon.gradle.toml-catalog")
}

javaPlatform { allowDependencies() }

dependencies {
    constraints {

        api(platform(project(":versions:gradle-plugin-bom")))

        // Kotlin
        api("org.jetbrains.kotlin:kotlin-test:2.4.20-RC")
        api("org.jetbrains.kotlin:kotlin-test-junit:2.4.20-RC")
        api("org.jetbrains.kotlin:kotlin-test-junit5:2.4.20-RC")
        api("org.jetbrains.kotlin:kotlin-stdlib:2.4.20-RC")
        api("org.jetbrains.kotlin:kotlin-reflect:2.4.20-RC")


        // Kotlinx
        api("org.jetbrains.kotlinx:kotlinx-serialization-core:1.10.0")
        api("org.jetbrains.kotlinx:kotlinx-serialization-json:1.10.0")
        api("org.jetbrains.kotlinx:kotlinx-serialization-cbor:1.10.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-reactor:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-core:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-core-js:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-core-jvm:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-core-linuxx64:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-core-iosarm64:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-core-iossimulatorarm64:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-core-wasm-js:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-core-wasm-wasi:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-test:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-reactor:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-reactive:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-test-js:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-test-wasm-js:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-test-wasm-wasi:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-android:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-swing:1.11.0")
        api("org.jetbrains.kotlinx:kotlinx-coroutines-jdk8:1.11.0")

        // Pinned to the 0.7.1 `0.6.x-compat` flavour: vanilla 0.7.1 moved
        // `kotlinx.datetime.Instant` into `kotlin.time` (now a typealias) and
        // ships no `kotlinx/datetime/Instant.class` on the runtime classpath.
        // Kotlin Dataframe's CSV / convert bytecode still references the old
        // class name and throws `NoClassDefFoundError: kotlinx/datetime/Instant`
        // at ingest. The `-0.6.x-compat` variant keeps the legacy class
        // alongside the new stdlib alias so both Dataframe (old call sites)
        // and IDK code (already-on-stdlib call sites) resolve. Move back to
        // plain 0.7.x once Dataframe ships a release compiled against
        // `kotlin.time.Instant`.
        api("org.jetbrains.kotlinx:kotlinx-datetime:0.7.1-0.6.x-compat")
        api("org.jetbrains.kotlinx:kotlinx-io-core:0.9.0")
        api("org.jetbrains.kotlinx:atomicfu:0.32.1")




        api("org.slf4j:slf4j-simple:2.0.17")

        // JUnit
        api("org.junit.jupiter:junit-jupiter-engine:5.13.4")

        // Kotest
        api("io.kotest:kotest-assertions-core:6.0.6")
        api("io.kotest:kotest-framework-engine:6.0.6")
        api("io.kotest:kotest-runner-junit5:6.0.6")
        api("io.kotest:kotest-property:6.0.6")

            // assertk
        api("com.willowtreeapps.assertk:assertk:0.28.1")


    }
}

// Ensure the generateTomlCatalog task runs before the publish task
tasks.withType<PublishToMavenRepository>().configureEach {
    dependsOn(tasks.named("generateTomlCatalog"))
}

tasks.withType<PublishToMavenLocal>().configureEach {
    dependsOn(tasks.named("generateTomlCatalog"))
}
/*

publishing {
    publications {
        create<MavenPublication>("bomCatalog") {
            //  only one publication
            groupId    = group.toString()
            artifactId = project.name
            version    = version.toString()

            // POM-only “platform” BOM:
            pom { packaging = "pom" }

            // attach the two TOML files as classifier artifacts
            artifact("build/tomlCatalog/sureCommonBom.versioned.toml") {
                builtBy(tasks.named("generateTomlCatalog"))
                extension  = "toml"
                classifier = "catalog-versioned"
            }
            artifact("build/tomlCatalog/sureCommonBom.toml") {
                builtBy(tasks.named("generateTomlCatalog"))
                extension  = "toml"
                classifier = "catalog-bom"
            }
        }
    }
}*/
